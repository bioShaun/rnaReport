...
iso_len_dis_path_size = command.get('mRNA-pdf-size', 'iso_len_dis_path_size')
gene_len_dis_path_size = command.get('mRNA-pdf-size', 'gene_len_dis_path_size')

mRNA_data_dict = dict(enrichment=enrichment_path, fastqc=fastqc_path,
                      mapping=mapping_path, quantification=quantification_path,
                      rseqc=rseqc_path, nr_assembly=nr_assembly_path)
...
for key, value in quantification_analysis_path.items():
    quantification_analysis_path[key] = os.path.join(
        ...
        expression_summary_dir, value)

# diff part
diff_analysis_path = dict(
    volcano_plot_path='ALL.Volcano_plot.pdf.png',
...
pdf_plots_size_dict = dict(
    ...
    iso_len_dis_path_size=iso_len_dis_path_size,
    gene_len_dis_path_size=gene_len_dis_path_size,
)
